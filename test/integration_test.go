...
func (p *TestPlugin) Execute(ctx context.Context, job *jobs.Job) (interface{}, error) {
	if p.ShouldFail {
...
func TestIntegration_SubmissionAndExecution(t *testing.T) {
	...
	// 1. Submit Job
	job, err := svc.CreateJob(ctx, "test-success", nil, nil, "medium", 3, "", "", "", "", "", 0, 0, "", nil, "", nil, "", 0)
	if err != nil {
...
func TestIntegration_RetryMechanism(t *testing.T) {
	...
	job, err := svc.CreateJob(ctx, "test-fail", nil, nil, "medium", 1, "", "", "", "", "", 0, 0, "", nil, "", nil, "", 0)
	if err != nil {
		t.Fatalf("failed to create job: %v", err)
	}
...
func TestIntegration_Scheduling(t *testing.T) {
	...
	job, err := svc.CreateJob(ctx, "test-scheduled", nil, nil, "medium", 3, "", "", "", runAt, "", 0, 0, "", nil, "", nil, "", 0)
	if err != nil {
